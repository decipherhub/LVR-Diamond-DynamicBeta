...
class ScaleToRange(nn.Module):
    def __init__(self, min_val, max_val):
        super(ScaleToRange, self).__init__()
...
class ActorCritic(nn.Module):
    ...
    def evaluate(self, state, action):
        if self.has_continuous_action_space:
            action_mean = self.actor(state.reshape(-1, self.state_dim))
        ...
        state_values = self.critic(state.reshape(-1, self.state_dim))
...
class PPO:
    def __init__(
        ...
        if pre_train is not None:
            self.policy.actor.load_state_dict(pre_train.state_dict())
